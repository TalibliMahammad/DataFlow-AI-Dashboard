...
"[project]/store/useAuthStore.ts [app-ssr] (ecmascript)", ((__turbopack_context__, module, exports) => {

const e = new Error("Could not parse module '[project]/store/useAuthStore.ts'\n\nExpected ident");
...
function AuthSignupForm() {
    ...
    const handleSubmit = async (e)=>{
        ...
        setIsLoading(true);
        try {
            // 3. Süni gecikmə (UI-ı görmək üçün)
            await new Promise((resolve)=>setTimeout(resolve, 2000));
            // 4. Store-dakı signup funksiyasını gözləyirik
            const success = await signup({
                fullName: name,
                email: email,
                password: password
            }, "manual");
            // Qeyd: Əgər signup funksiyan geriyə dəyər qaytarmırsa,
            // catch blokuna düşməsi üçün Firebase xətası fırlatmalıdır.
            if (success) {
            ...
            // 6. Hər iki halda loading-i dayandırırıq
            setIsLoading(false);
        }
    };
    const handleGoogleLogin = async ()=>{
...
//# sourceMappingURL=%5Broot-of-the-server%5D__6d1ecebc._.js.map